...
<button class="admin-backdrop" type="button" aria-label="Tutup navigasi" data-admin-backdrop></button>
<div class="admin-shell"><header class="admin-topbar"><button class="admin-menu-toggle" type="button" aria-label="Buka navigasi" aria-expanded="false" data-admin-menu><i></i><i></i><i></i></button><div><b><?= e($user['name']) ?></b><small><?= e($user['email']) ?></small></div><form method="post" action="<?= e(url('admin/logout')) ?>"><?= csrf_field() ?><button class="logout-button">Logout</button></form></header><main><?= $content ?></main></div>
